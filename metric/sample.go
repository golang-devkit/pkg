...
func ExamplePutRead(t *metrics) {
	if t == nil {
		return
	}
	// Example usage of putRead
	if err := t.putRead("exampleMethod", nil); err != nil {
		fmt.Printf("Error sending read metric: %v\n", err)
	}
}

func ExamplePutWrite(t *metrics) {
	if t == nil {
		return
	}
	// Example usage of putWrite
	if err := t.putWrite("exampleMethod", nil); err != nil {
		fmt.Printf("Error sending write metric: %v\n", err)
	}
}
