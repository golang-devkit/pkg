...
type SyncOption struct {
	UseFilenameOriginal bool // use original filename instead of timestamped filename
}

type FileWriter interface {
	Write(p []byte) (n int, err error)
	SyncFile(path string, opt ...SyncOption)
	Close()
}
...
func (f *file) SyncFile(path string, opt ...SyncOption) {
	// split path to dir and filename
	dir, filename := filepath.Split(path)
	if err := os.MkdirAll(dir, 0755); err != nil {
		log.Printf("create log directory error: %v", err)
	}
	// default sync filename options
	// not use original filename, add timestamp to filename
	// unless specified in sync file options
	useFilenameOriginal := false
	// parse options
	for _, opt := range opt {
		if opt.UseFilenameOriginal {
			useFilenameOriginal = true
		}
	}
	// if not use original filename, add timestamp to filename
	...
	//
	// Use os.OpenFile to append log to existing file
